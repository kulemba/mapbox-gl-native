...
- (void)setStyleURL:(nullable NSURL *)styleURL withMaxZoomLimit:(double)maxZoomLimit
{
    ...
    self.mbglMap.getStyle().loadURL([[styleURL absoluteString] UTF8String], std::floor(maxZoomLimit));
}

- (IBAction)reloadStyle:(__unused id)sender {
    MGLLogInfo(@"Reloading style.");
    NSURL *styleURL = self.styleURL;
    self.mbglMap.getStyle().loadURL("", _mbglMap->getStyle().getMaxZoomLimit());
    self.styleURL = styleURL;
}
