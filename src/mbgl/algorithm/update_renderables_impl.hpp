...
template <typename DataTiles, typename Renderables>
bool tryLoadingTile(const UnwrappedTileID& renderTileID,
              const OverscaledTileID& dataTileID,
              DataTiles& dataTiles,
              Renderables& renderables,
              std::set<OverscaledTileID>& retain,
              std::function<typename DataTiles::mapped_type(const OverscaledTileID&)> callback) {
    if (renderables.find(renderTileID) == renderables.end()) {
        auto it = dataTiles.find(dataTileID);
        if (it == dataTiles.end()) {
            if (auto data = callback(dataTileID)) {
                it = dataTiles.emplace(dataTileID, std::move(data)).first;
            }
...
std::map<UnwrappedTileID, Renderable> updateRenderables(DataTiles& dataTiles,
                                                        const IdealTileIDs& idealTileIDs,
                                                        const SourceInfo& info,
                                                        const uint8_t z,
                                                        std::set<OverscaledTileID>& retain,
                                                        std::function<typename DataTiles::mapped_type(const OverscaledTileID&)> tileCreatorCallback) {
        ...
        if (!tryTile(renderTileID, dataTileID, dataTiles, renderables)) {
            ...
            if (!covered) {
                ...
                for (overscaledZ = z - 1; overscaledZ >= info.minZoom; --overscaledZ) {
                    ...
                    if (tryTile(parentRenderTileID, parentDataTileID, dataTiles, renderables) || tryLoadingTile(parentRenderTileID, parentDataTileID, dataTiles, renderables, retain, tileCreatorCallback)) {
